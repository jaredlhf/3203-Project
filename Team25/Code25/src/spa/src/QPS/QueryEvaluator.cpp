...
QueryEvaluator::resolveSelectSynonyms(
...
QueryEvaluator::resolveClauses(
  ...
  // If there is a syn_err, sem_err or no match in the header, short-circuit and
  // return
  std::vector<Constants::ClauseResult> clauseResList;
  for (std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>> clauseRes :
       clauseResults) {
...
std::list<std::string>
QueryEvaluator::evaluate(ParserResponse response,
  ...
  // Edge case:: If resultSynonym is a BOOLEAN synonym, mark it before clause
  // resolution
  ...
  for (PatternClausePair ptClausePair : this->patternClauses) {
    std::shared_ptr<PatternClause> ptnClause =
        std::static_pointer_cast<PatternClause>(ptClausePair.second);
    clauseResults.push_back(
        ptnClause->resolve(pkbRetriever, ptClausePair.first));
  }
  ...
  if (hasBoolSyn) { // If select is BOOLEAN, only consider the clause result
                    // status
  ...
  std::set<std::string> ansSet =
      ResultFormatter(getResultNames(), finalResTable).getResults();
