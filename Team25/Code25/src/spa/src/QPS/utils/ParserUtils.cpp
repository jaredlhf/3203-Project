...
// std::unordered_set<std::string> DESIGN_ENTITIES = {Constants::STMT,
// Constants::READ, Constants::PRINT, Constants::CALL,
//             Constants::WHILE, Constants::IF, Constants::ASSIGN,
//             Constants::VARIABLE, Constants::CONSTANT, Constants::PROCEDURE};
std::unordered_set<std::string> DESIGN_ENTITIES = {
    "stmt", "read",   "print",    "call",     "while",
    "if",   "assign", "variable", "constant", "procedure"};
std::unordered_set<std::string> RELREF = {
    "Follows",  "Follows*", "Parent", "Parent*", "Uses",
    "Modifies", "Calls",    "Calls*", "Next",    "Next*"};
std::unordered_set<std::string> PROC_DESIGN_ENTITIES = {
    "assign", "call", "if", "while", "read", "print", "stmt", "procedure"};
...
bool ParserUtils::isValidNaming(const std::string &s) {
  ...
  // checks if first character of synonym name or variable name starts with a
  // letter
...
std::shared_ptr<Entity> ParserUtils::getValidWithRef(
    const std::string &s,
...
std::shared_ptr<Entity> ParserUtils::getValidEntRef(
    const std::string &s,
    const std::vector<std::shared_ptr<Synonym>> &declarations) {
...
std::shared_ptr<Entity> ParserUtils::getValidProcRef(
    const std::string &s,
    const std::vector<std::shared_ptr<Synonym>> &declarations) {
  ...
  if (isValidNaming(s)) {
    ...
    if (PROC_DESIGN_ENTITIES.find(syn->getKeyword()) ==
        PROC_DESIGN_ENTITIES.end()) {
...
std::shared_ptr<Entity> ParserUtils::getValidStmtRef(
    const std::string &s,
    const std::vector<std::shared_ptr<Synonym>> &declarations) {
...
std::shared_ptr<Synonym> ParserUtils::getValidDeclaration(
    const std::string &s,
    const std::vector<std::shared_ptr<Synonym>> &declarations) {
  if (!isValidNaming(s)) {
    return Synonym::create(Constants::SYNTAX_ERROR, "");
...
std::shared_ptr<Synonym> ParserUtils::getValidAttrRef(
    const std::string &s,
    const std::vector<std::shared_ptr<Synonym>> &declarations) {
  ...
  std::shared_ptr<Synonym> synonym =
      getValidDeclaration(tokens[0], declarations);
...
bool ParserUtils::isExpectedSynonym(std::shared_ptr<Entity> e,
                                    const std::string &s) {
  if (e->isSynonym()) {
