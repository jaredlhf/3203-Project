...
std::string QueryTokenizer::preprocess(const std::string &word) {
  ...
  for (int i = 0; i < word.length(); i++) {
    ...
    if (inQuotes && !std::isspace(c)) {
      ...
      continue;
      // Handle the case where * is part of the clause name or # is part of
      // attrname
    } else if (c == '*' || c == '#') {
      res += c;
      res += ' ';
    }
    // If char is not a whitespace or alphanumeric, insert a space inbetween,
    // else just append char
...
std::vector<std::string> QueryTokenizer::tokenize(const std::string &query) {
  ...
  auto words_begin = std::sregex_iterator(processedQuery.begin(),
                                          processedQuery.end(), words_regex);
