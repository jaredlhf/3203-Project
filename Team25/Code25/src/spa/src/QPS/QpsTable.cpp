...
std::shared_ptr<QpsTable>
QpsTable::create(std::vector<std::string> headers,
                 std::set<std::vector<std::string>> data) {
...
QpsTable::getDefaultSynErr() {
  return std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>(
      Constants::ClauseResult::SYN_ERR, QpsTable::create());
}

std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
QpsTable::getDefaultSemErr() {
  return std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>(
      Constants::ClauseResult::SEM_ERR, QpsTable::create());
}
...
QpsTable::getDefaultOk() {
  return std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>(
      Constants::ClauseResult::OK, QpsTable::create());
}

// QPSTABLE INSTANCE FUNCTIONS
std::vector<std::string> QpsTable::getHeaders() { return this->headers; }

std::set<std::vector<std::string>> QpsTable::getData() { return this->data; }

bool QpsTable::canAdd(std::vector<std::string> row) {
  return row.size() == this->headers.size();
}

void QpsTable::addRow(std::vector<std::string> row) { this->data.insert(row); }

/* Returns the set of answers in a column, or an empty set if the header idx is
 * not in range */
...
/* Returns the set of answers in a column, or an empty set if the header name is
 * not in table*/
std::set<std::string> QpsTable::getColResults(const std::string &headerName) {
  ptrdiff_t pos = find(this->headers.begin(), this->headers.end(), headerName) -
                  this->headers.begin();
...
bool QpsTable::hasOverlappingHeaders(std::shared_ptr<QpsTable> other) {
  ...
  for (std::string header : this->headers) {
    if (std::find(otherHeaders.begin(), otherHeaders.end(), header) !=
        otherHeaders.end()) {
      return true;
    }
  }
...
QpsTable::getCommonHeaderIndices(std::shared_ptr<QpsTable> other) {
...
/*
        Returns a vector of unique headers, with this->headers in order at the
   front, and other->headers at the back in the same order, with any duplicate
   headers removed.

        E.g. this->headers = {"x", "y", "z"}, other->headers = {"a", "x", "b",
   "y", "c"} returns {"x", "y", "z", "a", "b", "c"}
*/
std::vector<std::string>
QpsTable::getUniqueHeaders(std::shared_ptr<QpsTable> other) {
  std::vector<std::pair<int, int>> cmnHeaderIdx =
      this->getCommonHeaderIndices(other);
...
std::shared_ptr<QpsTable>
QpsTable::crossProduct(std::shared_ptr<QpsTable> other) {
  std::vector<std::string> resultHeaders = this->headers;
  std::vector<std::string> otherHeaders = other->headers;
  resultHeaders.insert(resultHeaders.end(), otherHeaders.begin(),
                       otherHeaders.end());
...
bool QpsTable::canMergeInnerJoinRows(
    std::vector<std::string> row1, std::vector<std::string> row2,
...
std::vector<std::string>
QpsTable::mergeInnerJoinRows(std::vector<std::string> row1,
...
std::shared_ptr<QpsTable> QpsTable::innerJoin(std::shared_ptr<QpsTable> other) {
  ...
  */
  ...
  // With "this" as the outer block, and "other" as the inner block, find rows
  // where cmn headers' value match
  for (std::vector<std::string> thisRow : this->data) {
    for (std::vector<std::string> otherRow : other->data) {
