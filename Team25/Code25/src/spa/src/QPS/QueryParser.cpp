...
QueryParser::validateSuchThatClause(std::vector<std::string> s) {
  ...
  if (s[1] != Constants::OPEN_BRACKET ||
      s[s.size() - 1] != Constants::CLOSE_BRACKET) {
    return {};
  }
...
std::vector<std::shared_ptr<Synonym>> QueryParser::validateSelectSynonyms(
  ...
  if (tokens[0] != OPEN_SELECT_BRACKET &&
      tokens[tokenLength - 1] != Constants::CLOSE_BRACKET) {
    return result;
  }
...
std::vector<std::shared_ptr<Synonym>> QueryParser::processDeclaration(
...
ParserResponse QueryParser::parseQueryTokens(std::vector<std::string> tokens) {
  ...
  while (ptr < tokenLength) {
    ...
    std::vector<std::shared_ptr<Synonym>> temp =
        processDeclaration(declaration, declared_synonyms);
    if (temp.empty()) {
  ...
  while (ptr < tokenLength) {
    ...
    if (hasSeenSelectToken) {
      ...
      // confirms its a semantic error, not a boolean query and not an open
      // bracket token
      ...
      // if theres a match, means there is a valid select synonym or boolean
      // query
      ...
      std::vector<std::shared_ptr<Synonym>> synonyms =
          validateSelectSynonyms(selectTokens, declarations);
      if (synonyms.empty()) {
        return generateSyntaxErrorResponse();
      }
  ...
  while (ptr < tokenLength) {
    ...
    if (isPattern && !isSuchThat && !isWith) {
      ...
      std::shared_ptr<Synonym> patternSynonym =
          ParserUtils::getValidDeclaration(patternTokens[0], declarations);
      if (ParserUtils::isSyntaxError(patternSynonym)) {
        return generateSyntaxErrorResponse();
      }
      std::vector<std::string> patternCond = {patternTokens.begin() + 1,
                                              patternTokens.end()};
      std::shared_ptr<PatternValidator> patternValidator =
          PatternValidator::create(patternSynonym, patternCond);
      PatternClausePair patternPair =
          patternValidator->verifyPattern(declarations);
    ...
    if (isSuchThat && !isPattern && !isWith) {
      ...
      std::vector<std::string> suchThatArgs =
          validateSuchThatClause(suchThatTokens);
      if (suchThatArgs.empty()) {
        return generateSyntaxErrorResponse();
      }

      std::shared_ptr<Relationship> relationshipValidator =
          Relationship::create(suchThatArgs[0], suchThatArgs[1],
                               suchThatArgs[2]);
      std::vector<std::shared_ptr<Entity>> refs =
          relationshipValidator->verifyRelationship(declarations);
      ...
      suchThatClauses.push_back(Clause::create(
          relationshipValidator->getKeyword(), firstRef, secondRef));
    }
    ...
    if (isWith && !isPattern && !isSuchThat) {
      ...
      std::shared_ptr<Entity> firstRef =
          ParserUtils::getValidWithRef(withTokens[0], declarations);
      std::shared_ptr<Entity> secondRef =
          ParserUtils::getValidWithRef(withTokens[2], declarations);

      if (ParserUtils::isSyntaxError(firstRef) ||
          ParserUtils::isSyntaxError(secondRef)) {
        return generateSyntaxErrorResponse();
      }
      if (ParserUtils::isSemanticError(firstRef) ||
          ParserUtils::isSemanticError(secondRef)) {
        hasSemanticError = true;
      }
      withClauses.push_back(
          Clause::create(Constants::WITH, firstRef, secondRef));
    }
