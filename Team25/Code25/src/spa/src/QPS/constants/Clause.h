...
public:
  ...
  resolve(std::shared_ptr<PkbRetriever> pkbRet);
  virtual bool isWrongArgs();
  virtual bool isSemInvalid();
  virtual bool isPatternClause();

  // Factory method for Clause subclasses
  static std::shared_ptr<Clause> create(const std::string clauseType,
                                        std::shared_ptr<Entity> arg1,
                                        std::shared_ptr<Entity> arg2);

  // Static functions
  static std::unordered_set<int>
  getEveryStmt(std::shared_ptr<PkbRetriever> pkbRet);
};
...
public:
  ...
  virtual std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
  resolve(std::shared_ptr<PkbRetriever> pkbRet) override;
};
