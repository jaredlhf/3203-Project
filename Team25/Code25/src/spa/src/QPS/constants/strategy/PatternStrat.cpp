...
PatternStrat::PatternStrat(std::string clauseKeyword,
...
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
PatternStrat::wildcardSoftwildcard() {
  std::shared_ptr<Wildcard> wcArg2 =
      std::static_pointer_cast<Wildcard>(this->arg2);
  ...
  for (const std::string &stNum : synMatches) {
    ...
    if (StringUtils::postFixInFullpostFix(
            StringUtils::createPostFixNotation(wcPattern),
            StringUtils::createPostFixNotation(pkbPattern))) { // can optimise
...
PatternStrat::wildcardConst() {
  ...
  for (const std::string &stNum : synMatches) {
    const std::string &pkbPattern = pkbRet->getAssignRhs(std::stoi(stNum));
    if (StringUtils::createPostFixNotation(arg2Val) ==
        StringUtils::createPostFixNotation(
            pkbPattern)) { // check if postfix notation for both expressions are
                           // the same
      resTable->addRow({stNum});
    }
  }
...
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
PatternStrat::constSoftwildcard() {
  std::shared_ptr<Wildcard> wcArg2 =
      std::static_pointer_cast<Wildcard>(this->arg2);
  const std::string &arg1Val =
      std::static_pointer_cast<Value>(this->arg1)->getVal();
...
PatternStrat::constConst() {
  ...
  for (int intVal : pkbStmtMatches) {
    const std::string &pkbPattern = pkbRet->getAssignRhs(intVal);
    if (StringUtils::createPostFixNotation(arg2Val) ==
        StringUtils::createPostFixNotation(pkbPattern)) {
      resTable->addRow({std::to_string(intVal)});
    }
  }
