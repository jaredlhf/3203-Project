...
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
WithStrat::createReturnStrategy() {
...
WithStrat::constConst() {
  ...
  return arg1Val == arg2Val ? QpsTable::getDefaultOk()
...
WithStrat::constSyn() {
  ...
  std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
      unfilteredS2Res = s2Syn->resolveAttrResult(pkbRet);

  // Filter and keep only results that match constant in arg1
  std::shared_ptr<QpsTable> constTable =
      QpsTable::create({s2Syn->getNameWithAttr()});
...
WithStrat::synConst() {
  ...
  std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
      unfilteredS1Res = s1Syn->resolveAttrResult(pkbRet);

  // Filter and keep only results that match constant in arg2
  std::shared_ptr<QpsTable> constTable =
      QpsTable::create({s1Syn->getNameWithAttr()});
...
WithStrat::synSyn() {
  ...
  std::shared_ptr<QpsTable> resTable =
      QpsTable::create({s1Syn->getName(), s1Syn->getNameWithAttr(),
