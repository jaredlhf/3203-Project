...
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
CallsStrat::createReturnStrategy() {
...
// Case: Calls(_, _)
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
CallsStrat::wildcardWildcard() {
  return pkbRet->getAllLeftCall().size() > 0 ? QpsTable::getDefaultOk()
                                             : QpsTable::getDefaultNoMatch();
}

// Case: Calls(_, 2)
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
CallsStrat::wildcardConst() {
...
CallsStrat::constWildcard() {
  ...
  return procs.count(arg1Val) > 0 ? QpsTable::getDefaultOk()
                                  : QpsTable::getDefaultNoMatch();
}

// Case: Calls(1, 2)
std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
CallsStrat::constConst() {
  const std::string &arg1Val =
      std::static_pointer_cast<Value>(this->arg1)->getVal();
  const std::string &arg2Val =
      std::static_pointer_cast<Value>(this->arg2)->getVal();
