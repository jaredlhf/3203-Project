...
public:
  ...
  enum class ClauseResult { SYN_ERR = 0, SEM_ERR = 1, NO_MATCH = 2, OK = 3 };

  // Functions for attrName
  static bool isAttrNameInt(const std::string &attrName);

  // ClauseResult related functions
  static Constants::ClauseResult getLowerBound(Constants::ClauseResult c1,
                                               Constants::ClauseResult c2);
  static Constants::ClauseResult
  getLowerBound(std::vector<Constants::ClauseResult> clauseList);
};