...
public:
  ...
  resolveSelectResult(std::shared_ptr<PkbRetriever> pkbRet);
  virtual std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
  resolveAttrResult(std::shared_ptr<PkbRetriever> pkbRet);

  // Static functions
  static std::shared_ptr<Synonym> create(const std::string &type,
...
public:
  ...
  virtual std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>>
  resolveSelectResult(std::shared_ptr<PkbRetriever> pkbRet) override;
};
