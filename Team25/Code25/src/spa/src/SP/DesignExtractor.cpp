...
void ModifiesExtractor::visit(std::shared_ptr<TNode> n, int lineNo) {
  if (isAssignNode(n)) {
    ...
    std::shared_ptr<ProcedureNode> p =
        std::dynamic_pointer_cast<ProcedureNode>(n);
...
void ModifiesExtractor::visit(std::shared_ptr<IfNode> ifs, int lineNo) {
  ...
  std::vector<std::shared_ptr<StmtNode>> ifStmts =
      ifs->getIfLst()->getStatements();
  std::vector<std::shared_ptr<StmtNode>> elseStmts =
      ifs->getElseLst()->getStatements();
  for (auto i : ifStmts) {
    visit(i, lineNo);
  }
...
void ModifiesExtractor::visit(std::shared_ptr<WhileNode> wh, int lineNo) {
  ...
  std::vector<std::shared_ptr<StmtNode>> whStmts =
      wh->getStmtLst()->getStatements();
  for (auto j : whStmts) {
    visit(j, lineNo);
  }
}
...
void ModifiesExtractor::visit(std::shared_ptr<ProcedureNode> p, int lineNo) {
  std::vector<std::shared_ptr<StmtNode>> pStmts =
      p->getStmtLst()->getStatements();
  for (auto j : pStmts) {
...
void FollowsStarExtractor::visit(std::shared_ptr<StmtLstNode> sl, int lineNo) {
  ...
  while (stmtLines.size() != 1) {
    std::unordered_set<int> follows =
        std::unordered_set<int>(stmtLines.begin() + 1, stmtLines.end());
    int key = stmtLines[0];
