...
std::shared_ptr<StmtParser>
StmtParser::createStmtParser(std::string firstToken, std::string secondToken) {
  if (Token::isValidName(firstToken) &&
      secondToken == SPConstants::EQUAL_TOKEN) {
...
std::shared_ptr<ParserDTO>
StmtParser::parseStmt(std::string firstToken, std::string secondToken,
...
std::shared_ptr<ParserDTO>
StmtParser::parseStmtLst(std::shared_ptr<SPParserUtils> utils,
                         std::shared_ptr<Tokenizer> tokenizer,
                         const std::string &proc) {
  ...
  do {
    std::shared_ptr<ParserDTO> stmtDTO = parseStmt(
        tokenizer->peek(), tokenizer->peekTwice(), utils, tokenizer, proc);
  ...
  std::shared_ptr<ParserDTO> resultDTO =
      std::make_shared<ParserDTO>(std::make_shared<StmtLstNode>(node), head);
  return resultDTO;
}

std::shared_ptr<ParserDTO>
AssignParser::parse(std::shared_ptr<SPParserUtils> utils,
                    std::shared_ptr<Tokenizer> tokenizer,
                    const std::string &proc) {
  ...
  ParserDTO resultDTO =
      ParserDTO(std::make_shared<AssignNode>(node), utils->getCurrentStmtNo());
  return std::make_shared<ParserDTO>(resultDTO);
}

std::shared_ptr<ParserDTO>
PrintParser::parse(std::shared_ptr<SPParserUtils> utils,
                   std::shared_ptr<Tokenizer> tokenizer,
                   const std::string &proc) {
  ...
  ParserDTO resultDTO =
      ParserDTO(std::make_shared<PrintNode>(node), utils->getCurrentStmtNo());
  return std::make_shared<ParserDTO>(resultDTO);
}

std::shared_ptr<ParserDTO>
WhileParser::parse(std::shared_ptr<SPParserUtils> utils,
                   std::shared_ptr<Tokenizer> tokenizer,
                   const std::string &proc) {
  ...
  std::shared_ptr<CFGWhileNode> cfgNode =
      std::make_shared<CFGWhileNode>(lineNo);
...
std::shared_ptr<ParserDTO> IfParser::parse(std::shared_ptr<SPParserUtils> utils,
                                           std::shared_ptr<Tokenizer> tokenizer,
                                           const std::string &proc) {
  int lineNo = utils->getCurrentStmtNo();
  ...
  shared_ptr<StmtLstNode> ifLstNode =
      std::dynamic_pointer_cast<StmtLstNode>(ifLstDTO->getNode());
  utils->expect(std::make_shared<RightBrace>());
  utils->expect(std::make_shared<Else>());
  utils->expect(std::make_shared<LeftBrace>());
  shared_ptr<ParserDTO> elseLstDTO = parseStmtLst(utils, tokenizer, proc);
  shared_ptr<StmtLstNode> elseLstNode =
      std::dynamic_pointer_cast<StmtLstNode>(elseLstDTO->getNode());
  utils->expect(std::make_shared<RightBrace>());
...
std::shared_ptr<ParserDTO>
ReadParser::parse(std::shared_ptr<SPParserUtils> utils,
                  std::shared_ptr<Tokenizer> tokenizer,
                  const std::string &proc) {
  ...
  ParserDTO resultDTO =
      ParserDTO(std::make_shared<ReadNode>(node), utils->getCurrentStmtNo());
  return std::make_shared<ParserDTO>(resultDTO);
}

std::shared_ptr<ParserDTO>
CallParser::parse(std::shared_ptr<SPParserUtils> utils,
                  std::shared_ptr<Tokenizer> tokenizer,
                  const std::string &proc) {
  ...
  ParserDTO resultDTO =
      ParserDTO(std::make_shared<CallNode>(node), utils->getCurrentStmtNo());
  return std::make_shared<ParserDTO>(resultDTO);
}