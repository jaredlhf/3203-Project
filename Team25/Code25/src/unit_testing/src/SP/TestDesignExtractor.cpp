...
using namespace std;
// sample test for testing purposes, actual design extractor is tested in the
// integration tests
SCENARIO("ExtractingNode") {
  ...
  std::shared_ptr<PkbPopulator> pkbPop =
      std::make_shared<PkbPopulator>(pkbPopulator);
  GIVEN("ModifiesExtractor") {
    ...
    WHEN("a assign node is visited parsed") {
      ...
      THEN("v should populate") {
        REQUIRE(msPointer->getVar(a.getLine()) ==
    ...
    WHEN("a read node is visited parsed") {
      ...
      THEN("v should populate") {
        REQUIRE(msPointer->getVar(r.getLine()) ==
