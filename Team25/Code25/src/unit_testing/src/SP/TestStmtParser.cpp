  ...
  GIVEN("A few string types") {
    ...
    WHEN("valid string") {
      THEN("it should create a StmtParser of the correct type") {
        REQUIRE(typeid(*StmtParser::createStmtParser(readStr, readStr2)) ==
                typeid(*std::make_shared<ReadParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(printStr, printStr2)) ==
                typeid(*std::make_shared<PrintParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(callStr, callStr2)) ==
                typeid(*std::make_shared<CallParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(whileStr, whileStr2)) ==
                typeid(*std::make_shared<WhileParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(ifStr, ifStr2)) ==
                typeid(*std::make_shared<IfParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(assignStr, assignStr2)) ==
                typeid(*std::make_shared<AssignParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(assignStrOverlap,
                                                     assignStrOverlap2)) ==
                typeid(*std::make_shared<AssignParser>()));
        REQUIRE(typeid(*StmtParser::createStmtParser(assignStrOverlap3,
                                                     assignStrOverlap4)) ==
                typeid(*std::make_shared<AssignParser>()));
      }
    }
    ...
    WHEN("invalid string") {
      THEN("it should throw an error") {
        REQUIRE_THROWS_AS(StmtParser::createStmtParser(invalidAssignVarStr,
                                                       invalidAssignVarStr2),
                          std::invalid_argument);
        REQUIRE_THROWS_AS(StmtParser::createStmtParser(invalidAssignTokenStr,
                                                       invalidAssignTokenStr2),
                          std::invalid_argument);
      }
    }
  }
...
SCENARIO("Testing AssignParser") {
  GIVEN("A few assign statements") {
    ...
    std::string longValidStatement =
        "read = y - 1 * f / s % f + 231 + (z) - (w + y);";
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<AssignNode> node =
          dynamic_pointer_cast<AssignNode>(resultDTO->getNode());
    ...
    WHEN("tokenizer has a statement with an invalid variable on the right hand "
         "side ") {
      std::shared_ptr<Tokenizer> t = std::make_shared<Tokenizer>();
  ...
  GIVEN("A few print statements") {
    ...
    WHEN("tokenizer has valid statement") {
      ...
      std::shared_ptr<PrintNode> node =
          dynamic_pointer_cast<PrintNode>(resultDTO->getNode());
      THEN("it should generate correct AST") { REQUIRE(node->getVar() == "y"); }
    }
  ...
  GIVEN("A few read statements") {
    ...
    WHEN("tokenizer has valid statement") {
      ...
      std::shared_ptr<ReadNode> node =
          dynamic_pointer_cast<ReadNode>(resultDTO->getNode());
      THEN("it should generate correct AST") { REQUIRE(node->getVar() == "y"); }
    }
...
SCENARIO("Testing IfParser") {
  GIVEN("A few if statements") {
    std::string shortValidStatement =
        "if (x>0) then { x = x + 1; } else { y = y + 1; }";
    std::string longValidStatement =
        "if ((x >0)&& (y < 5)) then { \n"
        "x = x + 1; \n"
        "} else { \n"
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<IfNode> node =
          dynamic_pointer_cast<IfNode>(resultDTO->getNode());
      THEN("it should generate correct AST") {
        REQUIRE(node->getCondExpr() == "x>0");
        std::shared_ptr<AssignNode> a = dynamic_pointer_cast<AssignNode>(
            node->getIfLst()->getStatements()[0]);
        REQUIRE(a->getVar() == "x");
        REQUIRE(a->getExpr() == "x+1");
        std::shared_ptr<AssignNode> a2 = dynamic_pointer_cast<AssignNode>(
            node->getElseLst()->getStatements()[0]);
        REQUIRE(a2->getVar() == "y");
    ...
    WHEN("tokenizer has long valid statement") {
      ...
      THEN("it should generate correct AST") {
        ...
        REQUIRE(a2->getVar() == "y");
        REQUIRE(a2->getExpr() == "y+1");
        std::shared_ptr<IfNode> ifs = dynamic_pointer_cast<IfNode>(
            node->getElseLst()->getStatements()[1]);
        REQUIRE(ifs->getCondExpr() == "x<5");
        std::shared_ptr<AssignNode> a3 = dynamic_pointer_cast<AssignNode>(
            ifs->getIfLst()->getStatements()[0]);
        REQUIRE(a3->getVar() == "x");
        REQUIRE(a3->getExpr() == "x*10");
        std::shared_ptr<AssignNode> a4 = dynamic_pointer_cast<AssignNode>(
            ifs->getElseLst()->getStatements()[0]);
        REQUIRE(a4->getVar() == "y");
  ...
  GIVEN("A few while statements") {
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<WhileNode> node =
          dynamic_pointer_cast<WhileNode>(resultDTO->getNode());
      THEN("it should generate correct AST") {
        REQUIRE(node->getCondExpr() == "x>0");
        std::shared_ptr<AssignNode> a2 = dynamic_pointer_cast<AssignNode>(
            node->getStmtLst()->getStatements()[0]);
        REQUIRE(a2->getVar() == "y");
        REQUIRE(a2->getExpr() == "y+1");
      }
    }
    ...
    WHEN("tokenizer has long valid statement") {
      ...
      THEN("it should generate correct AST") {
        ...
        REQUIRE(a2->getVar() == "y");
        REQUIRE(a2->getExpr() == "y+1");
        std::shared_ptr<IfNode> ifs2 =
            dynamic_pointer_cast<IfNode>(ifs->getElseLst()->getStatements()[1]);
        REQUIRE(ifs2->getCondExpr() == "x<5");
        std::shared_ptr<AssignNode> a3 = dynamic_pointer_cast<AssignNode>(
            ifs2->getIfLst()->getStatements()[0]);
        REQUIRE(a3->getVar() == "x");
        REQUIRE(a3->getExpr() == "x*10");
        std::shared_ptr<AssignNode> a4 = dynamic_pointer_cast<AssignNode>(
            ifs2->getElseLst()->getStatements()[0]);
        REQUIRE(a4->getVar() == "y");
        REQUIRE(a4->getExpr() == "y*2");
      }
    }
...
SCENARIO("Testing StmtParser") {
  GIVEN("A few assign statements") {
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<SPParserUtils> util = std::make_shared<SPParserUtils>(t);
      std::shared_ptr<ParserDTO> resultDTO =
          StmtParser::parseStmt(t->peek(), t->peekTwice(), util, t, "test");
      std::shared_ptr<AssignNode> node =
          dynamic_pointer_cast<AssignNode>(resultDTO->getNode());
      THEN("it should generate correct AST") {
        REQUIRE(node->getVar() == "x");
  ...
  GIVEN("A few if statements") {
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<IfNode> node =
          dynamic_pointer_cast<IfNode>(resultDTO->getNode());
      std::shared_ptr<CFGIfNode> cfgNode =
          dynamic_pointer_cast<CFGIfNode>(resultDTO->getCFGNode());
      THEN("it should generate correct AST") {
        REQUIRE(node->getCondExpr() == "x>0");
        std::shared_ptr<AssignNode> a = dynamic_pointer_cast<AssignNode>(
            node->getIfLst()->getStatements()[0]);
    ...
    WHEN("tokenizer has long valid statement") {
      ...
      THEN("it should generate correct CFG") {
        ...
        std::shared_ptr<CFGNode> then2CfgNode =
            if2CfgNode->getAllNextNodes()[0];
        std::shared_ptr<CFGNode> else2CfgNode =
            if2CfgNode->getAllNextNodes()[1];
  ...
  GIVEN("A few while statements") {
    ...
    std::string manyNestedWhileValidStatement =
        "while ((x != 0) && (y != 0)) { \n"
        "\tcount = count + 1;\n"
        "\tx = y + 2;\n"
        "\twhile (y > 2) {\n"
    ...
    WHEN("tokenizer has short valid statement") {
      ...
      std::shared_ptr<WhileNode> node =
          dynamic_pointer_cast<WhileNode>(resultDTO->getNode());
      std::shared_ptr<CFGWhileNode> cfgNode =
          dynamic_pointer_cast<CFGWhileNode>(resultDTO->getCFGNode());
      THEN("it should generate correct AST") {
        REQUIRE(node->getCondExpr() == "x>0");
        std::shared_ptr<AssignNode> a2 = dynamic_pointer_cast<AssignNode>(
            node->getStmtLst()->getStatements()[0]);
    ...
    WHEN("tokenizer has long valid statement") {
      ...
      THEN("it should generate correct CFG") {
        ...
        std::shared_ptr<CFGNode> thenCfgNode =
            loopCfgNode->getAllNextNodes()[0];
        std::shared_ptr<CFGNode> elseCfgNode =
            loopCfgNode->getAllNextNodes()[1];
        REQUIRE(thenCfgNode->getLineNo().size() == 1);
        REQUIRE(thenCfgNode->getLineNo()[0] == 3);
        REQUIRE(thenCfgNode->getAllNextNodes().size() == 1);
        std::shared_ptr<CFGNode> thenNextCfgNode =
            thenCfgNode->getAllNextNodes()[0];
        ...
        std::shared_ptr<CFGNode> thenNext2CfgNode =
            then2CfgNode->getAllNextNodes()[0];
        REQUIRE(thenNext2CfgNode->getLineNo().size() == 1);
        REQUIRE(thenNext2CfgNode->getLineNo()[0] == 1);
        REQUIRE(else2CfgNode->getLineNo().size() == 1);
        REQUIRE(else2CfgNode->getLineNo()[0] == 7);
        std::shared_ptr<CFGNode> elseNext2CfgNode =
            else2CfgNode->getAllNextNodes()[0];
    ...
    WHEN("tokenizer has many nested whiles statement") {
      ...
      THEN("it should generate correct AST") {
        std::shared_ptr<AssignNode> a1 = dynamic_pointer_cast<AssignNode>(
            node->getStmtLst()->getStatements()[0]);
        REQUIRE(node->getCondExpr() == "(x!=0)&&(y!=0)");
        REQUIRE(a1->getVar() == "count");
        REQUIRE(a1->getExpr() == "count+1");
        std::shared_ptr<AssignNode> a2 = dynamic_pointer_cast<AssignNode>(
            node->getStmtLst()->getStatements()[1]);
        REQUIRE(a2->getVar() == "x");
        REQUIRE(a2->getExpr() == "y+2");
        std::shared_ptr<WhileNode> w1 = dynamic_pointer_cast<WhileNode>(
            node->getStmtLst()->getStatements()[2]);
        std::shared_ptr<WhileNode> w2 = dynamic_pointer_cast<WhileNode>(
            w1->getStmtLst()->getStatements()[0]);
        std::shared_ptr<AssignNode> a3 = dynamic_pointer_cast<AssignNode>(
            w2->getStmtLst()->getStatements()[0]);
        REQUIRE(w2->getCondExpr() == "c<3");
        REQUIRE(a3->getVar() == "a");
        REQUIRE(a3->getExpr() == "b+c");
        std::shared_ptr<AssignNode> a4 = dynamic_pointer_cast<AssignNode>(
            w1->getStmtLst()->getStatements()[1]);
        REQUIRE(a4->getVar() == "y");
        REQUIRE(a4->getExpr() == "y-3");
        std::shared_ptr<AssignNode> a5 = dynamic_pointer_cast<AssignNode>(
            w1->getStmtLst()->getStatements()[2]);
        REQUIRE(a5->getVar() == "x");
        REQUIRE(a5->getExpr() == "x+z");

        std::shared_ptr<WhileNode> w3 = dynamic_pointer_cast<WhileNode>(
            node->getStmtLst()->getStatements()[3]);
        std::shared_ptr<AssignNode> a6 = dynamic_pointer_cast<AssignNode>(
            w3->getStmtLst()->getStatements()[0]);
