  ...
  GIVEN("New instance of modifies procedure store") {
    ...
    WHEN("One modifiesProc is added") {
      ...
      THEN("Procedure should be mapped to variable") {
        REQUIRE(mprocStore.getProc("x") ==
                std::unordered_set<std::string>({"sampleProc"}));
        REQUIRE(mprocStore.getVar("sampleProc") ==
    ...
    WHEN("Three modifiesProc are added") {
      ...
      THEN("There should be 3 procedures and 2 variables") {
        REQUIRE(mprocStore.getAllProc() ==
                std::unordered_set<std::string>({"proc1", "proc2", "proc3"}));
        REQUIRE(mprocStore.getAllVar() ==
      ...
      THEN("Procedures should be mapped to variables correctly") {
        ...
        REQUIRE(mprocStore.getProc("y") ==
                std::unordered_set<std::string>({"proc3"}));
        REQUIRE(mprocStore.getVar("proc1") ==
                std::unordered_set<std::string>({"x"}));
        REQUIRE(mprocStore.getVar("proc2") ==
                std::unordered_set<std::string>({"x"}));
        REQUIRE(mprocStore.getVar("proc3") ==
