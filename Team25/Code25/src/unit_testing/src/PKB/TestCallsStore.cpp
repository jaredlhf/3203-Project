	...
	GIVEN("New instance of calls store") {
		...
		WHEN("One calls is added") {
			...
			THEN("Left call should be mapped to right call") {
				REQUIRE(callsStore.getLeftProc("proc2") == std::unordered_set<std::string>({ "proc1" }));
				REQUIRE(callsStore.getRightProc("proc1") == std::unordered_set<std::string>({ "proc2" }));
			}
			...
			WHEN("New calls is added to existing relation") {
				...
				THEN("Calls should have two relations") {
					REQUIRE(callsStore.getLeftProc("proc2") == std::unordered_set<std::string>({ "proc1" }));
					REQUIRE(callsStore.getLeftProc("proc3") == std::unordered_set<std::string>({ "proc1" }));
					REQUIRE(callsStore.getRightProc("proc1") == std::unordered_set<std::string>({ "proc2", "proc3" }));
				}
			}
		}
		WHEN("Two calls are added") {
			...
			THEN("Left calls should be mapped to right calls correctly") {
				REQUIRE(callsStore.getLeftProc("proc2") == std::unordered_set<std::string>({ "proc1" }));
				REQUIRE(callsStore.getRightProc("proc1") == std::unordered_set<std::string>({ "proc2" }));
				REQUIRE(callsStore.getLeftProc("proc3") == std::unordered_set<std::string>({ "proc2" }));
				REQUIRE(callsStore.getRightProc("proc2") == std::unordered_set<std::string>({ "proc3" }));
			}
