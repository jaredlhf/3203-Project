	...
	GIVEN("New instance of uses procedure store") {
		...
		WHEN("One usesProc is added") {
			...
			THEN("Procedure should be mapped to variable") {
				REQUIRE(uprocStore.getProc("x") == std::unordered_set<std::string>({ "sampleProc" }));
		...
		WHEN("Three usesProc are added") {
			...
			THEN("There should be 3 procedures and 2 variables") {
				REQUIRE(uprocStore.getAllProc() == std::unordered_set<std::string>({ "proc1", "proc2", "proc3" }));
				REQUIRE(uprocStore.getAllVar() == std::unordered_set<std::string>({ "x", "y" }));
			}

			THEN("Procedures should be mapped to variables correctly") {
				REQUIRE(uprocStore.getProc("x") == std::unordered_set<std::string>({ "proc1", "proc2" }));
				REQUIRE(uprocStore.getProc("y") == std::unordered_set<std::string>({ "proc3" }));
