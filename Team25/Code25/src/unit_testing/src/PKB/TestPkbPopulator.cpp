...
SCENARIO("Working version of PkbPopulator") {
  GIVEN("Valid instances of empty stores") {
    ...
    WHEN("The PkbPopulator references empty stores") {
      ...
      THEN("Adding one procedure should increase the procedure store size by "
           "1") {
        REQUIRE(psPointer->size() == 0);
        pkbPop.addProc("sampleProc");
        REQUIRE(psPointer->size() == 1);
      }
      THEN("Adding one statement should increase the statement store size by "
           "1") {
        REQUIRE(ssPointer->size() == 0);
        pkbPop.addStmt(Constants::ASSIGN, 2);
        REQUIRE(ssPointer->size() == 1);
      }
      THEN("Adding one follows relationship should increase the follows store "
           "size by 1") {
      ...
      THEN("Adding one follows star should increase the follows star store "
           "size by 1") {
      ...
      THEN("Adding one modifes procedure should increase the modifiesProc "
           "store size by 1") {
      ...
      THEN("Adding one parentsStar should increase the parentsStar store size "
           "by 1") {
      ...
      THEN("Adding one uses procedure should increase the usesProc store size "
           "by 1") {
      ...
      THEN("Adding one call star should increase the callsStar store size by "
           "1") {
      ...
      THEN("Adding one print attribute should increase print attribute store "
           "size by 1") {
      ...
      THEN("Adding one read attribute should increase print attribute store "
           "size by 1") {
