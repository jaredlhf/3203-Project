...
SCENARIO("Mocking behavior of ParserResponse and PkbRetriever for QpsEvaluator "
         "test") {
  GIVEN("An instance of a QpsEvaluator class") {
    QueryEvaluator qe;

    WHEN("ParserResponse and PkbRetriever are populated with query for "
         "variables for only select queries") {
      VariableStore vs;
      ConstantStore cs;
      FollowsStore fs;
      ProcedureStore ps;
      ...
      std::shared_ptr<UsesStore> usesPointer =
          std::make_shared<UsesStore>(uses);
      ...
      std::shared_ptr<NextStore> nextPointer =
          std::make_shared<NextStore>(next);

      PkbRetriever pkbRet(vsPointer, csPointer, fsPointer, psPointer, ssPointer,
                          pattsPointer, fstarsPointer, mprocsPointer, msPointer,
                          pStarsPointer, parentsPointer, uprocsPointer,
                          usesPointer, callsPointer, cStarsPointer,
                          printAPointer, readAPointer, callAPointer,
                          nextPointer);
      ...
      THEN("When QpsEvaluator evaluates a singular select of SyntaxError, it "
           "should return SYN_ERR") {
        list<string> expected = {Constants::SYNTAX_ERROR};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::VARIABLE, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
        response.setSelectSynonyms(
            {Synonym::create(Constants::SYNTAX_ERROR, "v")});

        list<string> res =
            qe.evaluate(response, std::make_shared<PkbRetriever>(pkbRet));
        REQUIRE(res == expected);
      }

      THEN("When QpsEvaluator evaluates a singular select of SemanticError, it "
           "should return SEM_ERR") {
        ...
        response.setDeclarations({Synonym::create(Constants::VARIABLE, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
        response.setSelectSynonyms(
            {Synonym::create(Constants::SEMANTIC_ERROR, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of VariableSynonym, "
      ...
      THEN("When QpsEvaluator evaluates a singular select of ProcedureSynonym, "
           "it should return the right results") {
        ...
        response.setDeclarations({Synonym::create(Constants::VARIABLE, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
        response.setSelectSynonyms(
            {Synonym::create(Constants::PROCEDURE, "pr")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of ConstantSynonym, "
           "it should return the right results") {
        ...
        response.setDeclarations({Synonym::create(Constants::VARIABLE, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
        response.setSelectSynonyms(
            {Synonym::create(Constants::CONSTANT, "cs")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of StmtSynonym, it "
           "should return the right results") {
        list<string> expected = {"1", "10", "11", "12", "2", "3",
                                 "4", "5",  "6",  "7",  "8", "9"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::STMT, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of AssignSynonym, it "
           "should return the right results") {
        list<string> expected = {"1", "2"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::ASSIGN, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of CallSynonym, it "
           "should return the right results") {
        list<string> expected = {"3", "4"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::CALL, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of WhileSynonym, it "
           "should return the right results") {
        list<string> expected = {"5", "6"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::WHILE, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of IfSynonym, it "
           "should return the right results") {
        list<string> expected = {"7", "8"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::IF, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of PrintSynonym, it "
           "should return the right results") {
        list<string> expected = {"10", "9"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::PRINT, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates a singular select of ReadSynonym, it "
           "should return the right results") {
        list<string> expected = {"11", "12"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::READ, "x"),
                                  Synonym::create(Constants::VARIABLE, "v")});
      ...
      THEN("When QpsEvaluator evaluates multiple, it should return the right "
           "results") {
        list<string> expected = {"11 x", "11 y", "11 z",
                                 "12 x", "12 y", "12 z"};
        ParserResponse response;
      ...
      THEN("When QpsEvaluator evaluates multiple with repeating, it should "
           "return the right results") {
        list<string> expected = {"11 11 x", "11 11 y", "11 11 z",
                                 "12 12 x", "12 12 y", "12 12 z"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::READ, "rd"),
                                  Synonym::create(Constants::READ, "rd"),
                                  Synonym::create(Constants::VARIABLE, "v")});
        response.setSelectSynonyms({Synonym::create(Constants::READ, "rd"),
                                    Synonym::create(Constants::READ, "rd"),
                                    Synonym::create(Constants::VARIABLE, "v")});
    ...
    WHEN("ParserResponse and PkbRetriever are populated with suchthat and/or "
         "pattern") {
      VariableStore vs;
      ConstantStore cs;
      FollowsStore fs;
      ...
      THEN("When QpsEvaluator evaluates a followsSt clause, it returns the "
           "right result") {
        ...
        response.setSuchThatClauses({Clause::create(
            Constants::FOLLOWSST, Synonym::create(Constants::ASSIGN, "a1"),
            Synonym::create(Constants::STMT, "s2"))});
      ...
      THEN("When QpsEvaluator evaluates a follows clause, it returns the right "
           "result") {
        ...
        response.setSuchThatClauses({Clause::create(
            Constants::FOLLOWS, Synonym::create(Constants::STMT, "s1"),
            Synonym::create(Constants::STMT, "s2"))});
      ...
      THEN("When QpsEvaluator evaluates a parentSt clause, it returns the "
           "right result") {
        list<string> expected = {"2", "4"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::STMT, "s1"),
                                  Synonym::create(Constants::READ, "rd2")});
        response.setSelectSynonyms({Synonym::create(Constants::STMT, "s1")});
        response.setSuchThatClauses({Clause::create(
            Constants::PARENTST, Synonym::create(Constants::STMT, "s1"),
            Synonym::create(Constants::READ, "rd2"))});
      ...
      THEN("When QpsEvaluator evaluates a parent clause, it returns the right "
           "result") {
        ...
        response.setSuchThatClauses({Clause::create(
            Constants::PARENT, Synonym::create(Constants::STMT, "s1"),
            Synonym::create(Constants::ASSIGN, "a2"))});
      ...
      THEN("When QpsEvaluator evaluates a modifies clause, it returns the "
           "right result") {
        ...
        response.setSelectSynonyms({Synonym::create(Constants::STMT, "s1")});
        response.setSuchThatClauses({Clause::create(
            Constants::MODIFIES, Synonym::create(Constants::STMT, "s1"),
            Value::create("x"))});
      ...
      THEN("When QpsEvaluator evaluates a modifies clause with procedure, it "
        ...
        response.setSelectSynonyms(
            {Synonym::create(Constants::PROCEDURE, "s1")});
        response.setSuchThatClauses({Clause::create(
            Constants::MODIFIES, Synonym::create(Constants::PROCEDURE, "s1"),
            Value::create("x"))});
      ...
      THEN("When QpsEvaluator evaluates a modifies clause with procedure "
           "constant, it returns the right result") {
        list<string> expected = {"x", "y"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::VARIABLE, "v2")});
        response.setSelectSynonyms(
            {Synonym::create(Constants::VARIABLE, "v2")});
        response.setSuchThatClauses(
            {Clause::create(Constants::MODIFIES, Value::create("factorial"),
                            Synonym::create(Constants::VARIABLE, "v2"))});
      ...
      THEN("When QpsEvaluator evaluates a uses clause, it returns the right "
           "result") {
        ...
        response.setSelectSynonyms({Synonym::create(Constants::STMT, "s1")});
        response.setSuchThatClauses({Clause::create(
            Constants::USES, Synonym::create(Constants::STMT, "s1"),
            Value::create("x"))});
      ...
      THEN("When QpsEvaluator evaluates a uses clause with procedure, it "
           "returns the right result") {
        list<string> expected = {"factorial", "main"};
        ParserResponse response;
        ...
        response.setSelectSynonyms(
            {Synonym::create(Constants::PROCEDURE, "s1")});
        response.setSuchThatClauses({Clause::create(
            Constants::USES, Synonym::create(Constants::PROCEDURE, "s1"),
            Value::create("x"))});
      ...
      THEN("When QpsEvaluator evaluates a uses clause with procedure constant, "
        ...
        response.setSelectSynonyms(
            {Synonym::create(Constants::VARIABLE, "v2")});
        response.setSuchThatClauses(
            {Clause::create(Constants::USES, Value::create("beta"),
                            Synonym::create(Constants::VARIABLE, "v2"))});
      ...
      THEN("When QpsEvaluator evaluates a calls clause, it returns the right "
           "result") {
        ...
        response.setSelectSynonyms(
            {Synonym::create(Constants::PROCEDURE, "v2")});
        response.setSuchThatClauses({Clause::create(
            Constants::CALLS, Synonym::create(Constants::PROCEDURE, "v2"),
            Value::create("main"))});
      ...
      THEN("When QpsEvaluator evaluates a calls* clause, it returns the right "
        ...
        response.setSelectSynonyms(
            {Synonym::create(Constants::PROCEDURE, "v2")});
        response.setSuchThatClauses({Clause::create(
            Constants::CALLSST, Synonym::create(Constants::PROCEDURE, "v2"),
            Value::create("beta"))});
      ...
      THEN("When QpsEvaluator evaluates a pattern clause, it returns the right "
           "result") {
        list<string> expected = {"3", "7", "9"};
        ParserResponse response;
        ...
        response.setDeclarations({Synonym::create(Constants::ASSIGN, "a1")});
        response.setSelectSynonyms({Synonym::create(Constants::ASSIGN, "a1")});
        response.setPatternClauses(
            {{Synonym::create(Constants::ASSIGN, "a1"),
              Clause::create(Constants::PATTERN, Wildcard::create(),
                             Wildcard::create("x"))}});
      ...
      THEN("When QpsEvaluator evaluates 2 clauses, it returns the right "
           "result") {
        ...
        response.setPatternClauses(
            {{Synonym::create(Constants::ASSIGN, "a1"),
              Clause::create(Constants::PATTERN, Wildcard::create(),
                             Wildcard::create("y"))}});
        response.setSuchThatClauses({Clause::create(
            Constants::FOLLOWS, Synonym::create(Constants::ASSIGN, "a1"),
            Synonym::create(Constants::STMT, "s1"))});
      ...
      THEN("When QpsEvaluator evaluates BOOLEAN for a modifies clause with "
           "results, it should return TRUE") {
        list<string> expected = {"TRUE"};
        ParserResponse response;

        response.setDeclarations({Synonym::create(Constants::VARIABLE, "v2"),
                                  Synonym::create(Constants::BOOLEAN, "")});
      ...
      THEN("When QpsEvaluator evaluates BOOLEAN for a uses clause without "
           "matches, it returns the false") {
      ...
      THEN("When QpsEvaluator evaluates BOOLEAN without any clauses, it should "
           "return true") {
        list<string> expected = {"TRUE"};
        ParserResponse response;
...
SCENARIO("Mocking behavior of the resolveSelectSynonym function") {
  GIVEN("An instance of a QpsEvaluator class") {
    ...
    WHEN("PkbRetriever is populated with information of different type of "
         "synonyms") {
      VariableStore vs;
      ConstantStore cs;
      FollowsStore fs;
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on SyntaxErrorSynonym "
           "returns SYN_ERR") {
        std::shared_ptr<Synonym> s =
            Synonym::create(Constants::SYNTAX_ERROR, "x");
        std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>> res =
            qe.resolveSelectSynonyms({s},
                                     std::make_shared<PkbRetriever>(pkbRet));
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on SemanticErrorSynonym "
           "returns SEM_ERR") {
        std::shared_ptr<Synonym> s =
            Synonym::create(Constants::SEMANTIC_ERROR, "x");
        std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>> res =
            qe.resolveSelectSynonyms({s},
                                     std::make_shared<PkbRetriever>(pkbRet));
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on VariableSynonym "
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on ProcedureSynonym "
           "returns the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on ConstantSynonym "
           "returns the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on StmtSynonym returns "
           "the right results") {
        ...
        std::set<std::vector<std::string>> expectedData({{"1"},
                                                         {"2"},
                                                         {"3"},
                                                         {"4"},
                                                         {"5"},
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on AssignSynonym returns "
           "the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on CallSynonym returns "
           "the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on WhileSynonym returns "
           "the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on IfSynonym returns the "
           "right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on PrintSynonym returns "
           "the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on ReadSynonym returns "
           "the right results") {
      ...
      THEN("QpsEvaluator calling resolveSelectSynonym on multiple Synonyms "
           "returns the right results") {
        std::shared_ptr<Synonym> s1 = Synonym::create(Constants::READ, "rd");
        std::shared_ptr<Synonym> s2 =
            Synonym::create(Constants::CONSTANT, "cs");
        std::shared_ptr<Synonym> s3 = Synonym::create(Constants::PRINT, "pr");

        std::pair<Constants::ClauseResult, std::shared_ptr<QpsTable>> res =
            qe.resolveSelectSynonyms({s1, s2, s3},
                                     std::make_shared<PkbRetriever>(pkbRet));

        std::vector<std::string> expectedHeaders({"rd", "cs", "pr"});
        std::set<std::vector<std::string>> expectedData({{"11", "123", "9"},
                                                         {"11", "456", "9"},
                                                         {"11", "789", "9"},
                                                         {"11", "123", "10"},
                                                         {"11", "456", "10"},
...
TEST_CASE("resolveClauses function returns the right result for a clause with "
          "SYN_ERR") {
  QueryEvaluator qe;
...
TEST_CASE("resolveClauses function returns the right result for a clause for a "
          "valid set of clause tables with 3 related rows") {
  QueryEvaluator qe;
  ...
  std::shared_ptr<QpsTable> expectedTable =
      QpsTable::create(expectedHeader, expectedData);
