...
TEST_CASE("Formatting results for multiple results not same order as header returns the right results") {
	...
	std::set<std::string> expected = { "3 1 4", "y w z", "w3 w1 w4", "y3y w3w z3z" };
	std::set<std::string> res = ResultFormatter(resNames, resTable).getResults();
