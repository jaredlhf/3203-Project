...
TEST_CASE("Tokenize basic query with parantheses and newline") {
  ...
  vector<string> expected{"variable", "v", ";", "Select", "v", "such", "that",
                          "Follows*", "(", "6", ",",      "v", ")"};
  vector<string> res = qt.tokenize(testStr);
...
TEST_CASE("Tokenize complex query with wildcards") {
  string testStr = "assign a; while w;\nSelect a such that Parent*(w, a) "
                   "pattern a(_, _\"x+y\"_)";
  vector<string> expected{"assign", "a", ";",    "while",     "w",       ";",
                          "Select", "a", "such", "that",      "Parent*", "(",
                          "w",      ",", "a",    ")",         "pattern", "a",
                          "(",      "_", ",",    "_\"x+y\"_", ")"};
  vector<string> res = qt.tokenize(testStr);
...
TEST_CASE(
    "Tokenize complex query with whitespace and parantheses in wildcards") {
  string testStr = "assign a; while w;\nSelect a such that Parent*(w, a) "
                   "pattern a(_, _\"Follows*(x + y)\"_)";
  vector<string> expected{"assign",  "a",
                          ";",       "while",
                          "w",       ";",
                          "Select",  "a",
                          "such",    "that",
...
TEST_CASE("Tokenize query with attrNames") {
  ...
  vector<string> expected{"stmt",  "s1",    ";",    "Select", "s1",
                          ".",     "stmt#", "with", "s1",     ".",
                          "stmt#", "=",     "s1",   ".",      "stmt#"};
  vector<string> res = qt.tokenize(testStr);
