...
TEST_CASE("Synonym classes return the right values for Entity parent class "
          "virtual functions") {
  CallSynonym cl("cll");
...
TEST_CASE("Compare function returns the right results for Synonyms of the same "
          "type and name") {
  REQUIRE(Synonym::create(Constants::ASSIGN, "x")
              ->compare(Synonym::create(Constants::ASSIGN, "x")) == true);
  REQUIRE(Synonym::create(Constants::CALL, "x")
              ->compare(Synonym::create(Constants::CALL, "x")) == true);
}

TEST_CASE("Compare function returns false for Synonyms of different type") {
  REQUIRE(Synonym::create(Constants::CONSTANT, "x")
              ->compare(Synonym::create(Constants::ASSIGN, "x")) == false);
  REQUIRE(Synonym::create(Constants::CALL, "x")
              ->compare(Synonym::create(Constants::PROCEDURE, "x")) == false);
}

TEST_CASE("Compare function returns false for Synonyms of different name") {
  REQUIRE(Synonym::create(Constants::ASSIGN, "y")
              ->compare(Synonym::create(Constants::ASSIGN, "x")) == false);
  REQUIRE(Synonym::create(Constants::CALL, "x")
              ->compare(Synonym::create(Constants::CALL, "y")) == false);
}

// TESTS FOR STATIC SYNONYM CLASS FUNCTIONS
TEST_CASE("create function creates the right classes based on the keywords "
          "passed in") {
  REQUIRE(
      Synonym::create(Constants::STMT, "x")->matchesKeyword(Constants::STMT));
  REQUIRE(
      Synonym::create(Constants::READ, "x")->matchesKeyword(Constants::READ));
  REQUIRE(
      Synonym::create(Constants::PRINT, "x")->matchesKeyword(Constants::PRINT));
  REQUIRE(
      Synonym::create(Constants::CALL, "x")->matchesKeyword(Constants::CALL));
  REQUIRE(
      Synonym::create(Constants::WHILE, "x")->matchesKeyword(Constants::WHILE));
  REQUIRE(Synonym::create(Constants::IF, "x")->matchesKeyword(Constants::IF));
  REQUIRE(Synonym::create(Constants::ASSIGN, "x")
              ->matchesKeyword(Constants::ASSIGN));
  REQUIRE(Synonym::create(Constants::VARIABLE, "x")
              ->matchesKeyword(Constants::VARIABLE));
  REQUIRE(Synonym::create(Constants::CONSTANT, "x")
              ->matchesKeyword(Constants::CONSTANT));
  REQUIRE(Synonym::create(Constants::PROCEDURE, "x")
              ->matchesKeyword(Constants::PROCEDURE));
  REQUIRE(Synonym::create(Constants::BOOLEAN, "x")
              ->matchesKeyword(Constants::BOOLEAN));
  REQUIRE(Synonym::create(Constants::SYNTAX_ERROR, "x")
              ->matchesKeyword(Constants::SYNTAX_ERROR));
  REQUIRE(Synonym::create(Constants::SEMANTIC_ERROR, "x")
              ->matchesKeyword(Constants::SEMANTIC_ERROR));
}

TEST_CASE("create function with attrName creates the right classes based on "
  ...
  REQUIRE(Synonym::create(Constants::STMT, "x", attr1)
              ->matchesKeyword(Constants::STMT));
  REQUIRE(Synonym::create(Constants::STMT, "x", attr1)->matchesAttrName(attr1));

  REQUIRE(Synonym::create(Constants::READ, "x", attr2)
              ->matchesKeyword(Constants::READ));
  REQUIRE(Synonym::create(Constants::READ, "x", attr2)->matchesAttrName(attr2));

  REQUIRE(Synonym::create(Constants::PRINT, "x", attr3)
              ->matchesKeyword(Constants::PRINT));
  REQUIRE(
      Synonym::create(Constants::PRINT, "x", attr3)->matchesAttrName(attr3));

  REQUIRE(Synonym::create(Constants::CALL, "x", attr4)
              ->matchesKeyword(Constants::CALL));
  REQUIRE(Synonym::create(Constants::CALL, "x", attr4)->matchesAttrName(attr4));

  REQUIRE(Synonym::create(Constants::WHILE, "x", attr1)
              ->matchesKeyword(Constants::WHILE));
  REQUIRE(
      Synonym::create(Constants::WHILE, "x", attr1)->matchesAttrName(attr1));

  REQUIRE(Synonym::create(Constants::IF, "x", attr2)
              ->matchesKeyword(Constants::IF));
  REQUIRE(Synonym::create(Constants::IF, "x", attr2)->matchesAttrName(attr2));

  REQUIRE(Synonym::create(Constants::ASSIGN, "x", attr3)
              ->matchesKeyword(Constants::ASSIGN));
  REQUIRE(
      Synonym::create(Constants::ASSIGN, "x", attr3)->matchesAttrName(attr3));

  REQUIRE(Synonym::create(Constants::VARIABLE, "x", attr4)
              ->matchesKeyword(Constants::VARIABLE));
  REQUIRE(
      Synonym::create(Constants::VARIABLE, "x", attr4)->matchesAttrName(attr4));

  REQUIRE(Synonym::create(Constants::CONSTANT, "x", attr1)
              ->matchesKeyword(Constants::CONSTANT));
  REQUIRE(
      Synonym::create(Constants::CONSTANT, "x", attr1)->matchesAttrName(attr1));

  REQUIRE(Synonym::create(Constants::PROCEDURE, "x", attr2)
              ->matchesKeyword(Constants::PROCEDURE));
  REQUIRE(Synonym::create(Constants::PROCEDURE, "x", attr2)
              ->matchesAttrName(attr2));
}
...
TEST_CASE("All concrete synonym classes returns the right results for "
          "isStmtRef function") {
  StmtSynonym st("cll");
  ReadSynonym re("cll");
  PrintSynonym pr("cll");
...
TEST_CASE(
    "isBooleanSyn function returns the right result for the type of Synonym") {
  ...
  REQUIRE(Synonym::create(Constants::SYNTAX_ERROR, "x")->isBooleanSyn() ==
          false);
  REQUIRE(Synonym::create(Constants::SEMANTIC_ERROR, "x")->isBooleanSyn() ==
          false);
}
    ...
    WHEN("Populated with the attrname information") {
      ...
      THEN("When evaluating stmt.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::STMT, "s1", Constants::STMTNUM);

        REQUIRE(syn->resolveAttrResult(pkbRet).first == expectedStatus);
        REQUIRE(syn->resolveAttrResult(pkbRet).second->getHeaders() ==
                expectedTable->getHeaders());
        REQUIRE(syn->resolveAttrResult(pkbRet).second->getData() ==
      ...
      THEN("When evaluating asg.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::ASSIGN, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating if.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::IF, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating while.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::WHILE, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating read.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::READ, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating print.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::PRINT, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating call.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::CALL, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating proc.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::PROCEDURE, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating var.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::VARIABLE, "s1", Constants::STMTNUM);
      ...
      THEN("When evaluating const.stmt#") {
        ...
        std::shared_ptr<Synonym> syn =
            Synonym::create(Constants::CONSTANT, "s1", Constants::STMTNUM);
