...
SCENARIO("System testing between all components for Modifies") {
  GIVEN("The initialization and population of the PKB related classes") {
    ...
    WHEN("The SP and QPS objects are instantiated and a SIMPLE program is "
      ...
      THEN("For select all modified variables in one procedure, the right "
           "result is returned") {
      ...
      THEN("For select all modified variables in all procedures, the right "
           "result is returned") {
        list<string> expected = {"a", "ab", "e",  "ef", "fg", "g", "hi",
                                 "k", "m",  "mn", "o",  "om", "op"};
        list<string> res;

        string query =
            "procedure p; variable v;  Select v such that Modifies(p,v)";

        qps.query(query, res);
        REQUIRE(res == expected);
      }
      THEN("For select modified variable on a specific line, the right result "
           "is returned") {
      ...
      THEN("For select line where variable is modified , the right result is "
           "returned") {
      ...
      THEN("For select modified variables by if container statement, the right "
           "result is returned") {
      ...
      THEN("For select modified variables by while container statement, the "
